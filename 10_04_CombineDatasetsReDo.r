...
avus <- av[(av$StudyID %in% names(outcomedatasets)) & av$datatype!="Topline" & !is.na(av$majorclass),]
...
abortvarwpcts <- lapply(outcomemasterlist, function(x) wpct(x[,1]))
...
save(fcg, fg, file="EachFileExports/10-01_IndividualMasterPreReversals.rdata")
