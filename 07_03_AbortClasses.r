...
#Exclude
socialissues <- grepelf("abortion and gay marriage|same sex|same-sex|gay marriage and abortion|civil unions|marriage issues|social issues|gay rights|issues like abortion, immigration|abortion and stem cell|abortion/marriage", wordingcombinedabort) & !grepelf("make it more difficult", wordingcombinedabort)
...
idiosyncratic <- grepelf("political goals the religious right might have|what kind of stem cells|problem with women|how many abort|weygand|langevin|which activity on abortion|have you recently seen|too late to prevent pregnancy|girl had been given a drug|highest theyve been", wordingcombinedabort)
...
mostimportantvote <- grepelf("most important|mattered most|matters most", wordingcombinedabort) & grepelf("vot|choice|in dec", wordingcombinedabort) & !grepelf("not one of the|less important|one of most important", wordingcombinedabort)
whichparty <- ((grepelf("which cand|party|candidate|whose position", wordingcombinedabort) & grepelf("better|trust|handling|represent|close|prefer", wordingcombinedabort))|grepelf("clinton vs trump|if gore is elected", wordingcombinedabort)) & !grepelf("how much influence|first priority|when choosing", wordingcombinedabort)

importantvote <- (grepelf("importan|centrality|would you ever|if you agreed|differ|share|basis|likely|inclined|never vote|disagrees with|reason to|influence|effct|favorite candidate was", wordingcombinedabort) & grepelf("vote|choice|share|voting|support|cand|handl|vt|were to be chosen|choos", wordingcombinedabort) & !grepelf("senate to know|support the bill|anti-abortion cause|how important to you", wordingcombinedabort) & !(mostimportantvote|whichparty)) | grepelf("how important to you is the candidates position on abortion", wordingcombinedabort)
...
notifypar <- grepelf("notif|tell", wordingcombinedabort) & grepelf("parent|teen", wordingcombinedabort) & !parconsent & !grepelf("^when", wordingcombinedabort)
...
qcollapse <- lapply(avlister, function(x) data.frame(wording=as.character(x[1,"wordingsimp"]), responses=as.character(paste(names(rev(sort(table(unlist(strsplit(x[,"respssimp"], "; ", fixed=TRUE))))))[1:10], collapse="; ")), timesasked=dim(x)[1], firstasked=min(x[,"sd"], na.rm=TRUE), lastasked=max(x[,"ed"], na.rm=TRUE), type=as.character(x[1,"majorclass"]), subtype=as.character(x[1,"bigcats"]), specifictype=as.character(x[1,"wccat"]), totN=sum(as.numeric(x[,"N"]), na.rm=TRUE), surveyorgs=ntabs(x[,"surveyorg"]), surveysponsors=ntabs(x[,"sponsor"]), datatype=ntabs(x[,"datatype"]), stringsAsFactors=FALSE))

wccollapse <- lapply(avlisterwc, function(x) data.frame(specifictype=x[1,"wccat"], examplewording=as.character(names(rev(sort(table(as.character(x[,"wordingsimp"])))))[1]), timesasked=dim(x)[1], firstasked=min(x[,"sd"], na.rm=TRUE), lastasked=max(x[,"ed"], na.rm=TRUE), type=as.character(x[1,"majorclass"]), subtype=as.character(x[1,"bigcats"]), totN=sum(as.numeric(x[,"N"]), na.rm=TRUE), surveyorgs=ntabs(x[,"surveyorg"]), surveysponsors=ntabs(x[,"sponsor"]), datatype=ntabs(x[,"datatype"]), stringsAsFactors=FALSE))

bccollapse <- lapply(avlisterbc, function(x) data.frame(subtype=x[1,"bigcats"], examplewording=as.character(names(rev(sort(table(as.character(x[,"wordingsimp"])))))[1]), timesasked=dim(x)[1], firstasked=min(x[,"sd"], na.rm=TRUE), lastasked=max(x[,"ed"], na.rm=TRUE), type=as.character(x[1,"majorclass"]), specifictypes=ntabs(x[,"wccat"]), totN=sum(as.numeric(x[,"N"]), na.rm=TRUE), surveyorgs=ntabs(x[,"surveyorg"]), surveysponsors=ntabs(x[,"sponsor"]), datatype=ntabs(x[,"datatype"]), stringsAsFactors=FALSE))
